export const somniaPartners = {
  nodes: [
    {
      id: 'Somnia',
      name: 'Somnia',
      description:
        'Somnia is the fastest EVM-compatible layer one blockchain for a fully onchain world',
      logo: `${import.meta.env.BASE_URL}textures/somnia_logo.png`,
    },
    {
      id: 'Ankr RPC',
      name: 'Ankr RPC',
      description:
        'A provider of RPC nodes and subgraphs for blockchains. Supplies infrastructure for developers within the Somnia ecosystem.',
      logo: `${import.meta.env.BASE_URL}textures/ankr_logo.png`,
    },
    {
      id: 'Privy.io',
      name: 'Privy.io',
      description:
        'A platform enabling seamless wallet integration into Web3 applications, offering smooth onboarding and wallet management.',
      logo: `${import.meta.env.BASE_URL}textures/privy_logo.gif`,
    },
    {
      id: 'Thirdweb',
      name: 'Thirdweb',
      description:
        'A platform offering development tools, SDKs, and templates for building Web3 applications and Unity-based projects.',
      logo: `${import.meta.env.BASE_URL}textures/thirdweb.png`,
    },
    {
      id: 'YagaLabs',
      name: 'YagaLabs',
      description:
        'Creator of prominent NFT collections like Bored Ape Yacht Club and CryptoPunks. Collaborates with Somnia to enhance metaverse interoperability.',
      logo: `${import.meta.env.BASE_URL}textures/yagalabs.png`,
    },
    {
      id: 'Ormi',
      name: 'Ormi',
      description:
        'A blockchain data platform providing subgraphs, SQL engines, and analytics tools.',
      logo: `${import.meta.env.BASE_URL}textures/ormi.svg`,
    },
    {
      id: 'Protofire',
      name: 'Protofire',
      description:
        'Supplies Chainlink-compatible VRF and price feeds, as well as data indexing infrastructure via The Graph.',
      logo: `${import.meta.env.BASE_URL}textures/protofire.png`,
    },
    {
      id: 'Safe (Palmera)',
      name: 'Safe (Palmera)',
      description:
        'Offers infrastructure for managing multisig wallets and treasuries for on-chain organizations.',
      logo: `${import.meta.env.BASE_URL}textures/safe.jpg`,
    },
    {
      id: 'DIA',
      name: 'DIA',
      description:
        'A decentralized oracle network providing price feeds and verifiable randomness.',
      logo: `${import.meta.env.BASE_URL}textures/dia.webp`,
    },
    {
      id: 'Hyperlane',
      name: 'Hyperlane',
      description:
        'A framework for cross-chain interoperability, enabling asset transfers across over 100 blockchains.',
      logo: `${import.meta.env.BASE_URL}textures/hyperlane.svg`,
    },
    {
      id: 'Dune',
      name: 'Dune',
      description:
        'A blockchain analytics platform offering real-time visualizations and API access to on-chain data.',
      logo: `${import.meta.env.BASE_URL}textures/dune.jpg`,
    },
    {
      id: 'Sequence',
      name: 'Sequence',
      description: 'A Web3 gaming infrastructure provider offering smart wallets and Unity SDKs.',
      logo: `${import.meta.env.BASE_URL}textures/sequence.jpg`,
    },
    {
      id: 'Hemera’s Social Scan',
      name: 'Hemera’s Social Scan',
      description: 'A blockchain explorer with advanced indexing and semantic data layers.',
      logo: `${import.meta.env.BASE_URL}textures/social_scan.jpg`,
    },
    {
      id: 'Validation Cloud',
      name: 'Validation Cloud',
      description:
        'High-performance infrastructure provider for staking and nodes, ensuring network stability and security.',
      logo: `${import.meta.env.BASE_URL}textures/validation_cloud.png`,
    },
    {
      id: 'B-Harvest',
      name: 'B-Harvest',
      description:
        'A veteran validator in the Cosmos and EVM ecosystems with experience in developing DeFi protocols.',
      logo: `${import.meta.env.BASE_URL}textures/b_harvest.jpg`,
    },
    {
      id: 'Mirana',
      name: 'Mirana',
      description:
        'Mirana Ventures, a global investment fund focused on blockchain and Web3 projects, supporting startups in decentralized finance, gaming, and infrastructure with investments and partnerships.',
      logo: `${import.meta.env.BASE_URL}textures/mirana.jpg`,
    },
    {
      id: 'MML',
      name: 'MML',
      description:
        'Metaverse Markup Language (MML), a language developed by Somnia for defining interoperable objects and avatars in metaverse environments, enabling seamless digital asset integration.',
      logo: `${import.meta.env.BASE_URL}textures/mml.png`,
    },
    {
      id: 'Grillz Gang',
      name: 'Grillz Gang',
      description: 'NFT project focused on creating and managing non-fungible tokens.',
      logo: `${import.meta.env.BASE_URL}textures/grillz_gang.jpg`,
    },
    {
      id: 'Improbable',
      name: 'Improbable',
      description: 'Metaverse and gaming company supporting the development of Somnia’s ecosystem.',
      logo: `${import.meta.env.BASE_URL}textures/improbable.png`,
    },
    {
      id: 'MSquared',
      name: 'MSquared',
      description:
        'Interoperable metaverse network providing financial and technical support to Somnia.',
      logo: `${import.meta.env.BASE_URL}textures/msquared.png`,
    },
    {
      id: 'Playground',
      name: 'Playground',
      description: 'Web-based metaverse platform for user interaction and engagement.',
      logo: `${import.meta.env.BASE_URL}textures/playground.jpg`,
    },
    {
      id: 'Uprising',
      name: 'Uprising',
      description: 'Game publisher supporting the growth of Somnia’s gaming ecosystem.',
      logo: `${import.meta.env.BASE_URL}textures/uprising.jpeg`,
    },
    {
      id: 'Maelstrom',
      name: 'Maelstrom',
      description: 'Naval battle royale game built on blockchain technology.',
      logo: `${import.meta.env.BASE_URL}textures/maelstorm.jpg`,
    },
    {
      id: 'Standard',
      name: 'Standard',
      description: 'Decentralized exchange with an on-chain order book system.',
      logo: `${import.meta.env.BASE_URL}textures/standard.png`,
    },
    {
      id: 'ForU AI',
      name: 'ForU AI',
      description: 'AI-driven platform for decentralized identity solutions.',
      logo: `${import.meta.env.BASE_URL}textures/foruai.jpg`,
    },
    {
      id: 'Otomato',
      name: 'Otomato',
      description: 'No-code platform for creating AI agents.',
      logo: `${import.meta.env.BASE_URL}textures/otomato.jpg`,
    },
    {
      id: 'LootMogul',
      name: 'LootMogul',
      description: 'AI-powered sports technology platform for blockchain-based applications.',
      logo: `${import.meta.env.BASE_URL}textures/loot_mogul.webp`,
    },
    {
      id: 'Sogni AI',
      name: 'Sogni AI',
      description: 'Creative AI platform assisting content creators in the metaverse.',
      logo: `${import.meta.env.BASE_URL}textures/sogniai.jpg`,
    },
    {
      id: 'YOM',
      name: 'YOM',
      description: 'Cloud gaming network supporting blockchain-based gaming ecosystems.',
      logo: `${import.meta.env.BASE_URL}textures/yom.jpg`,
    },
    {
      id: 'Lucky Adventure Gambit',
      name: 'Lucky Adventure Gambit',
      description: 'Turn-based role-playing game integrated with blockchain technology.',
      logo: `${import.meta.env.BASE_URL}textures/lag.jpg`,
    },
    {
      id: 'Quills',
      name: 'Quills',
      description: 'NFT project focused on creating unique digital collectibles.',
      logo: `${import.meta.env.BASE_URL}textures/quills.jpg`,
    },
    {
      id: 'Galeon',
      name: 'Galeon',
      description: 'No-code platform for building blockchain-based games.',
      logo: `${import.meta.env.BASE_URL}textures/galeon.jpg`,
    },
    {
      id: 'QSTN',
      name: 'QSTN',
      description:
        'Survey-to-earn platform rewarding users for participating in blockchain-based surveys.',
      logo: `${import.meta.env.BASE_URL}textures/qstn.jpeg`,
    },
    {
      id: 'Glacis Labs',
      name: 'Glacis Labs',
      description: 'Cross-chain messaging platform for secure blockchain interactions.',
      logo: `${import.meta.env.BASE_URL}textures/glacis_labs.jpg`,
    },
    {
      id: 'Coin98',
      name: 'Coin98',
      description: 'Multi-chain wallet for managing assets across various blockchains.',
      logo: `${import.meta.env.BASE_URL}textures/coin98.png`,
    },
    {
      id: 'Netherak Demons',
      name: 'Netherak Demons',
      description: 'Dark fantasy role-playing game built on blockchain technology.',
      logo: `${import.meta.env.BASE_URL}textures/demons.jpg`,
    },
    {
      id: 'Dark Table CCG',
      name: 'Dark Table CCG',
      description: 'On-chain collectible card game leveraging blockchain for asset ownership.',
      logo: `${import.meta.env.BASE_URL}textures/dark_table.png`,
    },
    {
      id: 'Kraft Labs',
      name: 'Kraft Labs',
      description: 'Platform for creating hybrid casual games with blockchain integration.',
      logo: `${import.meta.env.BASE_URL}textures/kraft_labs.jpeg`,
    },
    {
      id: 'Zerion',
      name: 'Zerion',
      description: 'Non-custodial wallet for managing DeFi assets and investments.',
      logo: `${import.meta.env.BASE_URL}textures/zerion.png`,
    },
    {
      id: 'Particle Network',
      name: 'Particle Network',
      description: 'Wallet abstraction SDK and wallet provider for Web3 developers.',
      logo: `${import.meta.env.BASE_URL}textures/particle_network.png`,
    },
    {
      id: 'Mullet Cop The Game',
      name: 'Mullet Cop The Game',
      description: 'Blockchain-based simulation game set in a mall environment.',
      logo: `${import.meta.env.BASE_URL}textures/mcg.jpg`,
    },
    {
      id: 'Chunked',
      name: 'Chunked',
      description: 'Fully on-chain game leveraging blockchain for gameplay and assets.',
      logo: `${import.meta.env.BASE_URL}textures/chunked.webp`,
    },
    {
      id: 'GAM3S.GG',
      name: 'GAM3S.GG',
      description: 'Web3 gaming superapp aggregating blockchain-based gaming experiences.',
      logo: `${import.meta.env.BASE_URL}textures/g3.webp`,
    },
    {
      id: 'Luckysea',
      name: 'Luckysea',
      description: 'Web3 gaming superapp supporting decentralized gaming applications.',
      logo: `${import.meta.env.BASE_URL}textures/luckysea.jpg`,
    },
    {
      id: 'Blocksense',
      name: 'Blocksense',
      description: 'Permissionless oracle protocol for DeFi and blockchain applications.',
      logo: `${import.meta.env.BASE_URL}textures/blocksense.png`,
    },
    {
      id: 'Euclid Protocol',
      name: 'Euclid Protocol',
      description: 'Unified cross-chain liquidity layer for seamless asset transfers.',
      logo: `${import.meta.env.BASE_URL}textures/euclid.jpg`,
    },
  ],
  links: [
    { source: 'Somnia', target: 'Ankr RPC', color: 'blue' },
    { source: 'Somnia', target: 'Privy.io', color: 'violet' },
    { source: 'Somnia', target: 'Thirdweb', color: 'darkblue' },
    { source: 'Somnia', target: 'YagaLabs', color: 'pink' },
    { source: 'Somnia', target: 'Ormi', color: 'red' },
    //
    { source: 'Somnia', target: 'Protofire', color: 'blue' },
    { source: 'Somnia', target: 'Safe (Palmera)', color: 'violet' },
    { source: 'Somnia', target: 'DIA', color: 'darkblue' },
    { source: 'Somnia', target: 'Hyperlane', color: 'pink' },
    { source: 'Somnia', target: 'Dune', color: 'red' },
    //
    { source: 'Somnia', target: 'Sequence', color: 'blue' },
    { source: 'Somnia', target: 'Hemera’s Social Scan', color: 'violet' },
    { source: 'Somnia', target: 'Validation Cloud', color: 'darkblue' },
    { source: 'Somnia', target: 'B-Harvest', color: 'pink' },
    //
    { source: 'Somnia', target: 'MSquared', color: 'blue' },
    { source: 'Somnia', target: 'Mirana', color: 'violet' },
    { source: 'Somnia', target: 'MML', color: 'darkblue' },
    { source: 'Somnia', target: 'Grillz Gang', color: 'pink' },
    { source: 'Somnia', target: 'Improbable', color: 'red' },
    //
    { source: 'Somnia', target: 'Playground', color: 'violet' },
    { source: 'Somnia', target: 'Uprising', color: 'darkblue' },
    { source: 'Somnia', target: 'Maelstrom', color: 'red' },
    //
    { source: 'Somnia', target: 'Standard', color: 'blue' },
    { source: 'Somnia', target: 'ForU AI', color: 'violet' },
    { source: 'Somnia', target: 'Otomato', color: 'darkblue' },
    { source: 'Somnia', target: 'LootMogul', color: 'pink' },
    { source: 'Somnia', target: 'Sogni AI', color: 'red' },
    //
    { source: 'Somnia', target: 'YOM', color: 'blue' },
    { source: 'Somnia', target: 'Lucky Adventure Gambit', color: 'violet' },
    { source: 'Somnia', target: 'Quills', color: 'darkblue' },
    { source: 'Somnia', target: 'Galeon', color: 'pink' },
    { source: 'Somnia', target: 'QSTN', color: 'red' },
    //
    { source: 'Somnia', target: 'Glacis Labs', color: 'blue' },
    { source: 'Somnia', target: 'Coin98', color: 'violet' },
    { source: 'Somnia', target: 'Netherak Demons', color: 'darkblue' },
    { source: 'Somnia', target: 'Dark Table CCG', color: 'pink' },
    { source: 'Somnia', target: 'Kraft Labs', color: 'red' },
    //
    { source: 'Somnia', target: 'Zerion', color: 'blue' },
    { source: 'Somnia', target: 'Particle Network', color: 'violet' },
    { source: 'Somnia', target: 'Mullet Cop The Game', color: 'darkblue' },
    { source: 'Somnia', target: 'Chunked', color: 'pink' },
    { source: 'Somnia', target: 'GAM3S.GG', color: 'red' },
    //
    { source: 'Somnia', target: 'Luckysea', color: 'blue' },
    { source: 'Somnia', target: 'Blocksense', color: 'violet' },
    { source: 'Somnia', target: 'Euclid Protocol', color: 'darkblue' },
    //
    //
    //
    //
    { source: 'Thirdweb', target: 'Safe (Palmera)', color: 'whitesmoke' },
    { source: 'Uprising', target: 'Maelstrom', color: 'whitesmoke' },
    { source: 'YagaLabs', target: 'MSquared', color: 'whitesmoke' },
  ],
};
